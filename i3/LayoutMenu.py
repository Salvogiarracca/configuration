...
SCREENLAYOUT_PATH = "/home/giuseppe/.screenlayout/"
...
class LayoutMenuWindows(Gtk.ApplicationWindow):
    ...
    def __init__(self):
        Gtk.Window.__init__(self, title="Layout Menu")
        ...
        self.treeview.connect("row-activated", signal_handler, self.treeview.get_selection())


def signal_handler(treeview, path, column, param):
    (model, iter) = param.get_selected()
    os.system("notify-send 'Layout selected:' '" + model[iter][0] + "'")
    os.system("exec '" + SCREENLAYOUT_PATH + model[iter][0] + "'")
